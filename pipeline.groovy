pipeline {
    ...
    environment {
        ...
        KUBECTL_HOME = '/Users/a667253/Downloads/google-cloud-sdk/bin/kubectl'
        BUILD_DATE = new Date().format('yyyy-MM-dd')
        IMAGE_TAG = "${BUILD_DATE}-${BUILD_NUMBER}"
        IMAGE_NAME = 'orders' // Variable for the image name
        DOCKER_USERNAME = 'tinhch' // Variable for Docker Hub username
    ...
    stages {
        ...
        stage('Build Maven') {
            steps {
                checkout([$class: 'GitSCM', credentialsId: 'githubpwd', branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/TinhhCH/order-string.git']]])
        ...
        stage('Deploy to k8s') {
            steps {
                withKubeConfig([credentialsId: 'kubectlpwd', serverUrl: 'https://127.0.0.1:62281']) {
                    script {
